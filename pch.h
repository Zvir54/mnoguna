...
template<typename T>
void Set<T>::add(T x) // äîäàºì åëåìåíò .
{
...
template<typename T>
void Set<T>::remove(T x) // âèëó÷àºì åëåìåíò
{
...
template<typename T>
void Set<T>::clear() // Î÷èùóºì âåñü ìàñèâ
{
    for (int i = 0; i < position; ++i)
    {
        arr[i] = 0;
...
template<typename T>
bool Set<T>::Check(T x) // Ïåðåâ³ðêà íàÿâíîñò³ åëåìåíòà â ìíîæèí³
{
    bool check = false;
    for (int i = 0; i < position; ++i)
    {
...
template<typename T>
int Set<T>::set_size() //Ðîçì³ð ìíîæèíè
{
    return position;  // ïîâåðòàº ê³ëüê³ñòü åëåìåíò³â ÿê³ âæå º â ìíîæèí³
}
template<typename T>
Set<T> Set<T>::union_(Set& S) //Îáºäíàííÿ ìíîæèí
{
    Set un = S;
    for (int i = 0; i < position; ++i)
    {
...
template<typename T>
Set<T> Set<T>::intersect(Set& S) // Ïåðåòèí ìíîæèí
{
    Set in;
    for (int i = 0; i < position; ++i)
    {
...
template<typename T>
Set<T> Set<T>::difference(Set& S) // Ð³çíèöÿ äâîõ ìíîæèí
{
    Set dif;
    for (int i = 0; i < position; ++i)
    {
...
template<typename T>
void Set<T>::print(ostream& os) // âèâ³ä â ïîò³ê
{
    for (int i = 0; i < position; ++i)
    {
        os << arr[i] << ' ';
